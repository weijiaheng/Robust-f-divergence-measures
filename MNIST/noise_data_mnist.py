...
class MNIST_(data.Dataset):
    ...
    def _load_noise_label(self, is_train):
        ...
        if is_train:
            ...
            num = int(r*10)
            fname = f"MNIST_noise_r0_{num}.pt"
...
def peer_data_train(batch_size, img_size=(32, 32)):
    peer_dataset = MNIST_(root=root, train=True, valid=False, test=False, noisy=True, transform=transform_train)
...
def peer_data_val(batch_size, img_size=(32, 32)):
    peer_dataset = MNIST_(root=root, train=False, valid=True, test=False, noisy=True, transform=transform_train)
...
def peer_data_test(batch_size, img_size=(32, 32)):
    peer_dataset = MNIST_(root=root, train=False, valid=False, test=True, noisy=False, transform=transform_train)
...
train_dataset_noisy = MNIST_(root=root, train=True, valid=False, test=False, noisy=True, transform=transform_train)
        
train_loader_noisy = torch.utils.data.DataLoader(dataset=train_dataset_noisy, batch_size=batch_size, shuffle=True)

valid_dataset_noisy = MNIST_(root=root, train=False, valid=True, test=False, noisy=True, transform=transform_train)
        
valid_loader_noisy = torch.utils.data.DataLoader(dataset = valid_dataset_noisy, batch_size = batch_size, shuffle = False)

test_dataset = MNIST_(root=root, train=False, valid=False, test=True, noisy=False, transform=transform_test)
...
noise_file = f"MNIST_noise_r0_{int(r*10)}.pt"
key = 'noise_label_train'
(x_train, y_train), (x_test, y_test) = keras.datasets.mnist.load_data()
clean_label = y_train
dirty_label = torch.load(os.path.join('./noise_label', noise_file))
transition_matrix = confusion_matrix(clean_label, dirty_label[key])/6000
