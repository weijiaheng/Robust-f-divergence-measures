...
def train(train_loader, peer_loader, model, optimizer, epoch):
    ...
    for i, (idx, input, target) in enumerate(train_loader):
        ...
        if epoch >= warmup_epoch:
            ...
            # Bias correction of uniform noise case for the activation term
            for k in range(10):
            ...
            # Bias correction of uniform noise case for the conjugate term
            for k in range(10):
...
def main(writer):
    ...
    for epoch in range(num_epochs):
        ...
        if f_div_value >= max_f:
            max_f = f_div_value
            torch.save(model_prob, './trained_models/BIAS_UNIFORM' + str(args.r) + '_' + str(args.s) + '_' + str(args.divergence) + '_' + str(args.warmup))
...
if __name__ == '__main__':
    # Save statistics
    print("Begin:")
    writer1 = csv.writer(open(f'BIAS_UNIFORM_result_{r}_{div}_{args.warmup}.csv','w'))
    writer1.writerow(['Epoch', 'Training Acc', f'Val_Noisy_Acc', 'Test_ACC', 'f_div', 'f_test'])
    main(writer1)
    evaluate('./trained_models/BIAS_UNIFORM' + str(args.r) + '_' + str(args.s) + '_' + str(args.divergence) + '_' + str(args.warmup))
