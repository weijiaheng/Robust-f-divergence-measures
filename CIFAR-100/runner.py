...
class ProbLossStable(nn.Module):
    ...
    def forward(self, outputs, labels):
        return self._nllloss( self._softmax(outputs)  + self._eps, labels )
...
def main(writer):
#    model_prob = resnet_cifar18_pre(num_classes=100).cuda()
    ...
    for epoch in range(num_epochs):
        ...
        # Below we provide three learning rate settings which are used for all experiments in CIFAR-100
...
#            learning_rate = 0.000001
        ...
        test_acc = test(model=model_prob, test_loader=test_loader_, is_test = False)
